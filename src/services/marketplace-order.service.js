...
export async function priceCart(cart, session = null, strict = false) {
  ...
  for (const item of cart?.items || []) {
    try {
      ...
      items.push({ ...item.toObject(), productName: 'Unavailable item', error: error.message });
    }
  }
...
async function buildQuote(userId, body, session = null) {
  if (body.paymentMethod && body.paymentMethod !== 'cod') throw new ApiError(400, 'Only Cash on Delivery is available');
  ...
  const quote = { ...priced, ...totals, totalAmount: money(priced.subtotal - totals.discount + totals.deliveryCharge + totals.tax), shippingAddress, paymentMethod: 'cod' };
  ...
  return { cart, quote: { ...quote, quoteToken } };
}
...
export async function placeOrder(userId, body, key) {
  ...
  try {
    return await transaction(async session => {
      ...
      const [order] = await Order.create([{ ...quote, orderId: `ORD-${year}-${String(counter.value).padStart(6, '0')}`, userId, idempotencyKey: key, businessInfo: businessInfo(body.businessInfo), statusHistory: [{ status: 'pending', at: new Date(), actor: userId }] }], { session });
...
export async function changeOrderStatus(id, userId, status, admin = false) {
  return transaction(async session => {
